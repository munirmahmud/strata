 ...
 */
class Strata_recent_post extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'tp-recent-post', esc_html__( 'Strata Recent Post', 'strata' ), $widget_opt );
	}
	...
	function widget( $args, $instance ) {
		...
		?>
		<ul class="unstyled clearfix tp-recent-post-widget">
			<?php
			if ( $loop->have_posts() ):
				while ( $loop->have_posts() ):
					...
					<li>
						...
						<div class="post-info float-right w-75">
							<div class="post-meta tp-font-alt text-uppercase tp-mb-10">
								<span class="post-author"><?php echo esc_html__( 'Posted by','strata' ) ?> <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) ?>"><?php echo get_the_author(); ?></a></span>
			...
			<?php else: ?>
				<div class="nopost_message">
					<p><?php echo esc_html__( 'No Post Available', 'strata' ) ?></p>';
				</div>
			<?php endif; ?>  
		</ul>
		<?php
	...
	function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_html__( 'Recent Posts', 'strata' );
		}
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php esc_html_e( 'Number Of Posts:', 'strata' ); ?></label>
